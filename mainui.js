...
const DateNavigator = ({ selectedDate, onDateSelect, referenceToday, theme, t, isRTL, language }) => {
    ...
    return (
        <View style={styles.dateNavContainer(theme, isRTL)}>
            <View style={styles.dateNavHeader(isRTL)}>
                ...
                <Text style={styles.dateNavMonthText(theme, isRTL)}>{monthYearString}</Text>
            ...
            <View style={styles.weekContainer(isRTL)}>
                {weekDays.map((day, index) => <Text key={index} style={styles.weekDayText(theme, isRTL)}>{day}</Text>)}
            </View>
                    ...
                    return (
                        <TouchableOpacity key={index} onPress={() => onDateSelect(date)} disabled={isFutureDate}>
                            <View style={[ styles.dateCircle, isDaySelected && { backgroundColor: theme.primary, borderRadius: 20 } ]}>
                                <Text style={[ styles.dateText(theme, isRTL), isDaySelected && styles.activeText(theme), isFutureDate && styles.disabledDateText(theme) ]}>
                                    {date.getDate()}
                                </Text>
                            </View>
                        </TouchableOpacity>
    ...
    return (
        <View style={styles.nutrientRowContainer}>
            <View style={styles.nutrientRowHeader(isRTL)}>
                {isRTL ? (
                    <>
                        <Text style={styles.nutrientRowValue(theme, isRTL)}>{valueText}</Text>
                        <Text style={styles.nutrientRowLabel(theme, isRTL)}>{label}</Text>
                    </>
                ) : (
                    <>
                        <Text style={styles.nutrientRowLabel(theme, isRTL)}>{label}</Text>
            ...
            {routes.map((route) => {
                ...
                return (
                    <TouchableOpacity key={route.key} style={[styles.tabItem, { width: TAB_WIDTH, zIndex: 1 }]} onPress={onPress}>
                        ...
                        <Animated.Text style={[styles.tabText(theme, isRTL), textAnimatedStyle]}>{options.tabBarLabel}</Animated.Text>
                    </TouchableOpacity>
                );
            })}
        </View>
...
const commonStackOptions = (theme, isRTL) => ({ 
  headerStyle: { backgroundColor: theme.card, elevation: 0, shadowOpacity: 0 }, 
  headerTintColor: theme.textPrimary, 
  headerTitleStyle: { fontWeight: 'bold', fontFamily: isRTL ? 'Cairo-Regular' : undefined }, 
...
function DiaryStackNavigator({ setHasProgress, theme, t, isRTL, language }) { 
  return ( 
    <DiaryStack.Navigator screenOptions={commonStackOptions(theme, isRTL)}>
...
function ReportsStackNavigator({ theme, language }) { 
  ...
  return ( 
    <ReportsStack.Navigator screenOptions={commonStackOptions(theme, isRTL)}>
...
function ProfileStackNavigator({ theme, t, onThemeChange, appLanguage, isRTL }) {
  return (
    <ProfileStack.Navigator screenOptions={commonStackOptions(theme, isRTL)}>
...
// تم تغيير اسم المكون الأصلي ليعمل بشكل داخلي فقط
function AppNavigator({ appLanguage }) {
...
// هذا المكون هو "الواجهة" الجديدة، يقوم بتحميل الخط وتطبيق RTL ثم يعرض التطبيق
function MainUIScreen(props) {
  ...
  useEffect(() => {
    ...
    if (I18nManager.isRTL !== isArabic) {
        ...
        // ملاحظة مهمة: في الـ APK النهائي قد تحتاج لإعادة تحميل التطبيق
        // لتطبيق تغيير الاتجاه بشكل كامل باستخدام Updates.reloadAsync()
    }
  }, [appLanguage]);
  ...
  if (!fontsLoaded) {
    return null; // لا تعرض أي شيء حتى يتم تحميل الخط
  }
...
const styles = StyleSheet.create({ 
    ...
    card: (theme, isRTL) => ({ backgroundColor: theme.card, borderRadius: 20, padding: 20, marginBottom: 15 }), 
    dateNavContainer: (theme, isRTL) => ({ marginVertical: 10, backgroundColor: theme.card, borderRadius: 20, paddingVertical: 15, paddingHorizontal: 10 }), 
    ...
    dateNavMonthText: (theme, isRTL) => ({ flex: 1, fontSize: 18, fontWeight: 'bold', color: theme.textPrimary, textAlign: 'center', marginHorizontal: 10, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    weekContainer: (isRTL) => ({ flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-around', marginBottom: 10 }),
    weekDayText: (theme, isRTL) => ({ fontSize: 14, color: theme.textSecondary, fontWeight: '500', width: 40, textAlign: 'center', fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    ...
    dateText: (theme, isRTL) => ({ fontSize: 16, color: theme.textPrimary, fontWeight: '600', fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    ...
    remainingCaloriesText: (theme, isRTL) => ({ fontSize: 42, fontWeight: 'bold', color: theme.textPrimary, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    remainingLabel: (theme, isRTL) => ({ fontSize: 14, color: theme.textSecondary, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    ...
    sectionTitle: (theme, isRTL) => ({ fontSize: 22, fontWeight: 'bold', color: theme.textPrimary, textAlign: isRTL ? 'right' : 'left', marginBottom: 0, flexShrink: 1, fontFamily: isRTL ? 'Cairo-Regular' : undefined, writingDirection: isRTL ? 'rtl' : 'ltr' }),
    sectionDescription: (theme, isRTL) => ({ fontSize: 14, color: theme.textSecondary, textAlign: isRTL ? 'right' : 'left', fontFamily: isRTL ? 'Cairo-Regular' : undefined, writingDirection: isRTL ? 'rtl' : 'ltr' }),
    ...
    mealSectionTitle: (theme, isRTL) => ({ fontSize: 22, fontWeight: 'bold', color: theme.textPrimary, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    mealSectionTotalCalories: (theme, isRTL) => ({ fontSize: 16, color: theme.textSecondary, fontWeight: '600', fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    ...
    macroSummaryText: (theme, isRTL) => ({ fontSize: 13, color: theme.textSecondary, fontWeight: '600', fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    ...
    smartAddButtonText: (theme, isRTL) => ({ color: theme.white, fontSize: 18, fontWeight: 'bold', fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    ...
    readOnlyBannerText: (theme, isRTL) => ({ color: theme.white, fontSize: 14, fontWeight: 'bold', flex: 1, textAlign: isRTL ? 'right' : 'left', fontFamily: isRTL ? 'Cairo-Regular' : undefined, writingDirection: isRTL ? 'rtl' : 'ltr' }), 
    ...
    nutrientRowLabel: (theme, isRTL) => ({ fontSize: 16, color: theme.textPrimary, fontWeight: '600', fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    nutrientRowValue: (theme, isRTL) => ({ fontSize: 14, color: theme.textSecondary, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    tabBarContainer: (theme, isRTL) => ({ position: 'absolute', bottom: 0, left: 0, right: 0, height: 70, flexDirection: 'row', backgroundColor: theme.tabBarBackground }),
    ...
    tabText: (theme, isRTL) => ({ position: 'absolute', color: theme.tabBarIcon, fontSize: 12, fontWeight: '400', fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    ...
    modalView: (theme, isRTL) => ({ width: '90%', maxHeight: '80%', backgroundColor: theme.background, borderRadius: 20, padding: 0, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4, elevation: 5, overflow: 'hidden' }), 
    ...
    modalTitle: (theme, isRTL) => ({ fontSize: 20, fontWeight: 'bold', color: theme.textPrimary, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    searchContainer: (isRTL) => ({ flexDirection: isRTL ? 'row-reverse' : 'row', padding: 15, backgroundColor: 'transparent' }), 
    searchInput: (theme, isRTL) => ({ flex: 1, height: 50, backgroundColor: theme.background, borderRadius: 10, paddingHorizontal: 15, fontSize: 16, textAlign: isRTL ? 'right' : 'left', color: theme.textPrimary, fontFamily: isRTL ? 'Cairo-Regular' : undefined, writingDirection: isRTL ? 'rtl' : 'ltr' }), 
    ...
    foodName: (theme, isRTL) => ({ fontSize: 16, color: theme.textPrimary, fontFamily: isRTL ? 'Cairo-Regular' : undefined, writingDirection: isRTL ? 'rtl' : 'ltr' }), 
    emptyText: (theme, isRTL) => ({ textAlign: 'center', marginTop: 50, fontSize: 16, color: theme.textSecondary, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    dashboardGridContainer: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 15, flexWrap: 'wrap', rowGap: 15, }, 
    smallCard: (theme, isRTL) => ({ width: '48.5%', backgroundColor: theme.card, borderRadius: 20, padding: 15, minHeight: 120, justifyContent: 'space-between' }), 
    ...
    smallCardTitle: (theme, isRTL) => ({ fontSize: 16, fontWeight: '600', color: theme.textPrimary, [isRTL ? 'marginRight' : 'marginLeft']: 8, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    smallCardValue: (theme, isRTL) => ({ fontSize: 28, fontWeight: 'bold', color: theme.textPrimary, textAlign: isRTL ? 'right' : 'left', fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    smallCardSubValue: (theme, isRTL) => ({ fontSize: 14, color: theme.textSecondary, textAlign: isRTL ? 'right' : 'left', marginTop: -5, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    ...
    stepsCardCountText: (theme, isRTL) => ({ fontSize: 22, fontWeight: 'bold', color: theme.textPrimary, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    stepsCardGoalText: (theme, isRTL) => ({ fontSize: 13, color: theme.textSecondary, marginTop: 2, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    ...
    foodLogItemName: (theme, isRTL) => ({ fontSize: 16, fontWeight : '600', color: theme.textPrimary, flex: 1, textAlign: isRTL ? 'right' : 'left', fontFamily: isRTL ? 'Cairo-Regular' : undefined, writingDirection: isRTL ? 'rtl' : 'ltr' }), 
    foodLogItemCalories: (theme, isRTL) => ({ fontSize: 14, fontWeight: '500', color: theme.primary, [isRTL ? 'marginRight' : 'marginLeft']: 8, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    foodLogItemMacros: (isRTL) => ({ flexDirection: isRTL ? 'row-reverse' : 'row', alignItems: 'center', flexWrap: 'wrap', marginTop: 6, }), 
    macroText: (theme, isRTL) => ({ fontSize: 13, color: theme.textSecondary, [isRTL ? 'marginLeft' : 'marginRight']: 15, marginBottom: 4, fontFamily: isRTL ? 'Cairo-Regular' : undefined }), 
    ...
    previewCounterText: (theme, isRTL) => ({ color: theme.primary, fontWeight: 'bold', fontSize: 12, fontFamily: isRTL ? 'Cairo-Regular' : undefined }),
});
